pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
            def currentService = sh(script: 'kubectl get service nextjs-service -o json', returnStdout: true).trim()
            def currentDeployment = sh(script: 'kubectl get deployment nextjs-deployment -o json', returnStdout: true).trim()
            def currentIngress = sh(script: 'kubectl get ingress nextjs-ingress -o json', returnStdout: true).trim()
